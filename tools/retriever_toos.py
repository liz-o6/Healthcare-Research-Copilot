        ...
        for doc in files:
            try:
                if doc.suffix == ".txt":
                    ...
                    console.print(
                        f"[yellow]⚠ Unsupported document type: {doc}[/yellow]"
                    )
            except Exception as e:
                console.print(f"[yellow]⚠ Failed to load '{doc}': {e}[/yellow]")
...
async def retriever_tool(retriever: BaseRetriever, queries: List[str]) -> List[str]:
    """
